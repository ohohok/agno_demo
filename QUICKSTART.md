...
启动服务后，访问 http://localhost:7777 可以使用 AgentOS 自带的 Web 聊天界面。
...
```
agno_demo/
├── agno_agent.py          # 主应用文件（FastAPI 服务）
├── chat_cli.py            # 命令行交互模式
├── test_agno_agent.py     # 单元测试
├── api_examples.py        # API 调用示例
├── pyproject.toml         # 项目配置和依赖
└── ZHIPU_SETUP.md         # 智谱 AI 配置指南
```
...
在 `agno_agent.py` 中修改模型配置：

```python
model=OpenAIChat(
    id="glm-4",  # 可选: glm-4-flash, glm-4, glm-4-plus, glm-4v
...
A: 在 `agno_agent.py` 中的 Agent 配置里添加工具：
```python
